...
func InitTrans(locale string) (err error) {
	...
	if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
		...
		// 注册一个获取json tag的自定义方法 否则返回信息使用的是结构体中字段的形式
		...
		v.RegisterStructValidation(SignUpParamStructLevelValidation, model.ParamsSignUp{})
...
func SignUpParamStructLevelValidation(sl validator.StructLevel) {
	su := sl.Current().Interface().(model.ParamsSignUp)
